...
import type { ReceptionistBrand } from './brand';
...
export const PLANS: Record<PlanId, {
  label: string;
  perMinute: number;
  description: (brand: ReceptionistBrand) => string;
  ...
  after_hours: {
    label: 'After Hours',
    perMinute: 0.42,
    description: (brand) => `${brand} answers calls outside your business hours.`,
  ...
  '247': {
    label: '24/7',
    perMinute: 0.47,
    description: (brand) => `${brand} answers every call, day or night.`,
